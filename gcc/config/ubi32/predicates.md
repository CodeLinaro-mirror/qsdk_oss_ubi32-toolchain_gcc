...
(define_predicate "ubi32_move_operand"
...
(define_predicate "ubi32_symbolic_address_operand"
...
(define_predicate "ubi32_moveai_operand"
  ...
  switch (GET_CODE (op))
    ...
    case CONST_INT:
      /* For v4 and earlier we can only handle 31 bits but
         we can handle the full 32 bits for v5 and v6.  */
      if (ubi32_v5)
	return true;
...
(define_predicate "ubi32_fdpic_register_operand"
  (match_code "reg")
{
  unsigned int regno;
...
(define_predicate "ubi32_fdpic_got_1_offset_operand"
  (match_code "unspec")
{
  if (! TARGET_FDPIC)
    return false;
...
(define_predicate "ubi32_fdpic_got_4_offset_operand"
  (match_code "unspec")
{
  if (! TARGET_FDPIC)
    return false;
...
(define_predicate "ubi32_arith_operand"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
  return (ubi32_move_operand (op, mode)
	  && ! ubi32_symbolic_address_operand (op, mode)
	  && (! CONST_INT_P (op)
	      || satisfies_constraint_I (op)));
})

(define_predicate "ubi32_arith_operand_dot1"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
  return (ubi32_move_operand (op, mode)
	  && ! ubi32_symbolic_address_operand (op, mode)
...
(define_predicate "ubi32_arith_operand_dot2"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
  return (ubi32_move_operand (op, mode)
	  && ! ubi32_symbolic_address_operand (op, mode)
...
(define_predicate "ubi32_compare_operand"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
 return (ubi32_move_operand (op, mode)
	  && ! ubi32_symbolic_address_operand (op, mode)
...
(define_predicate "ubi32_compare_operator"
  (match_code "compare"))

(define_predicate "ubi32_and_or_si3_operand"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
  return (ubi32_arith_operand (op, mode)
	  || (CONST_INT_P (op)
...
(define_predicate "ubi32_and_or_hi3_operand"
  (match_code "subreg, reg, const_int, lo_sum, mem")
{
  return (ubi32_arith_operand (op, mode)
	  || (CONST_INT_P (op)
...
(define_predicate "ubi32_mem_or_address_register_operand"
...
(define_predicate "ubi32_data_register_operand"
...
(define_predicate "ubi32_address_register_operand"
  (match_code "subreg, reg")
{
  unsigned int regno;
...
(define_predicate "ubi32_acc_lo_register_operand"
  (match_code "subreg, reg")
{
  unsigned int regno;
...
(define_predicate "ubi32_acc_hi_register_operand"
  (match_code "subreg, reg")
{
  unsigned int regno;
...
(define_predicate "ubi32_acc_register_operand"
  (match_code "subreg, reg")
{
  unsigned int regno;
...
(define_predicate "ubi32_call_address_operand"
  (match_code "symbol_ref, subreg, reg")
{
  return (GET_CODE (op) == SYMBOL_REF || REG_P (op));
})

(define_special_predicate "ubi32_cc_register_operand"
  (and (match_code "reg")
       (match_test "REGNO (op) == CC_REGNUM")))

;; Even data register or acc0_hi/acc1_hi
(define_predicate "ubi32_dbl_acc_register_operand"
  (match_code "subreg, reg")
{
  unsigned int regno;
