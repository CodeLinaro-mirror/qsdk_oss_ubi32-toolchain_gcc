/* Subroutines for insn-output.c for Ubi32
...
/* FIXME -- Compare with other archs. */
#include "config.h"
#include "system.h"
#include "coretypes.h"
#include "tm.h"
...
static int ubi32_reloc_rw_mask (void);
static tree ubi32_handle_fndecl_attribute (tree *, tree, tree, int, bool *);
static void ubi32_function_prologue (FILE *);
static void ubi32_function_epilogue (FILE *);
static section * ubi32_function_rodata_section (tree);
static int ubi32_address_cost (rtx, machine_mode, addr_space_t, bool);
static bool ubi32_fixed_condition_code_regs (unsigned int *,
						unsigned int *);
...
/* Nonzero if this chip supports the Ubi32 v6.1 ISA.  */
int ubi32_v61 = 1;

/* Nonzero if this chip is big-endian.  */
int ubi32_big_endian = 0;

/* Scheduling rate in terms of expected concurrent threads.  */
int ubi32_sched_rate = 0;

/* ACC hi and low register numbers.  Default to big-endian.  */
unsigned int ubi32_acc0_hi_regnum = 24;
unsigned int ubi32_acc0_lo_regnum = 25;
unsigned int ubi32_acc1_hi_regnum = 26;
unsigned int ubi32_acc1_lo_regnum = 27;

/* Which instruction set architecture to use.  */
enum ubi32_processor_type ubi32_arch = UBI32_V61;

/* Valid attributes:
   naked - don't generate function prologue/epilogue and `ret' command.  */
/* FIXME -- Check attributes. */
const struct attribute_spec ubi32_attribute_table[] =
{
  /* { name, min_len, max_len, decl_req, type_req, fn_type_req, handler } */
  { "naked", 0, 0, true,  false, false, false,
    ubi32_handle_fndecl_attribute, NULL },
...
/* Register to class map -- Big endian.  */
enum reg_class const ubi32_regclass_map[FIRST_PSEUDO_REGISTER] =
{
  DATA_REGS,		/*  0 - d0  */
  DATA_REGS,		/*  1 - d1  */
  DATA_REGS,		/*  2 - d2  */
...
ubi32_option_override (void)
{
  ...
  if (strcmp (ubi32_sched_rate_name, "1") == 0)
    ubi32_sched_rate = 1;
  else if (strcmp (ubi32_sched_rate_name, "2") == 0)
    ubi32_sched_rate = 2;
  else if (strcmp (ubi32_sched_rate_name, "3") == 0)
    ubi32_sched_rate = 3;
  else if (strcmp (ubi32_sched_rate_name, "4") == 0)
    ubi32_sched_rate = 4;
  else
    ubi32_sched_rate = 0;
  ...
  /* There is no single unaligned SI op for PIC code.  Sometimes we
     ...
     See ubi32_assemble_integer for details.  */
  if (TARGET_FDPIC)
    targetm.asm_out.unaligned_op.si = 0;

  /* If we haven't been given an explicit function alignment then we might
     want to set one.  If we're optimizing for size we don't want to
     increase our footprint so we do nothing, while the v1 and v2 ISA only
     applied to CPUs that have no I-cache and thus don't gain from cache
     line aligning functions.  */
  if (align_functions == 0 && (! optimize_size) && ubi32_v3)
    align_functions = 32;
...
ubi32_conditional_register_usage (void)
{
  ...
  /* If we're running on a v1 or v2 ISA then there's only one accumulator
     register.  By marking the ACC1 registers as "fixed" they become
     invisible to the register allocator.  */
  if (! ubi32_v3)
    {
      fixed_regs[ubi32_acc1_hi_regnum] = 1;
      fixed_regs[ubi32_acc1_lo_regnum] = 1;
    }

  /* If we're running on a v1, v2 or v3 ISA then by convention we make A0
     hold a fixed value of zero so that we can move from data registers to
     address registers without always taking a pipeline hazard.  */
  if (! ubi32_v4)
...
/* We have some number of optimizations that don't really work for the Ubi32
   architecture so we deal with them here.  */

void
ubi32_option_optimization (int level ATTRIBUTE_UNUSED, int size ATTRIBUTE_UNUSED)
{
...
static int
ubi32_reloc_rw_mask (void)
{
  return TARGET_FDPIC ? 3 : 0;
}

/* Print register X using operand code CODE to assembly language output file
   FILE.  */

static void
ubi32_print_reg (FILE *file, unsigned int regno)
{
  /* For ISA v1 and v2 we have to handle use MAC_HI and MAC_LO instead of
     ACC0_HI and ACC0_LO.  */
  if (! ubi32_v3)
    {
      if (regno == ubi32_acc0_hi_regnum)
	{
	  fprintf (file, "mac_hi");
	  return;
	}
      else if (regno == ubi32_acc0_lo_regnum)
	{
	  fprintf (file, "mac_lo");
	  return;
	}
...
void
ubi32_print_operand (FILE *file, rtx x, int code)
{
  switch (code)
    {
    case 'A':
      /* Identify the correct accumulator to use.  */
      if (REGNO (x) == ubi32_acc0_hi_regnum || REGNO (x) == ubi32_acc0_lo_regnum)
	fprintf (file, "acc0");
      else if (REGNO (x) == ubi32_acc1_hi_regnum || REGNO (x) == ubi32_acc1_lo_regnum)
	fprintf (file, "acc1");
      else
	ubi32_print_reg (file, REGNO (x));
      break;
      ...
      else if (GET_CODE (x) == UNSPEC)
	{
	  switch (XINT (x, 1))
	    ...
	    case UNSPEC_FDPIC_GOT_4:
	      fprintf (file, "%%got_lo(");
	      ubi32_print_operand_address (file, XVECEXP (x, 0, 0));
	      fprintf (file, ")");
	      break;
	...
	case SUBREG:
	  ubi32_print_reg (file, subreg_regno (x));
	  break;
	...
	case LO_SUM:
	case CONST:
	  ubi32_print_operand_address (file, x);
	  break;
	...
	case HIGH:
	  fprintf (file, "#%%hi(");
	  ubi32_print_operand_address (file, XEXP (x, 0));
	  fprintf (file, ")");
	  break;
...
void
ubi32_print_operand_address (FILE *file, rtx addr)
{
  int mode_size;

  mode_size = GET_MODE_SIZE (output_memory_reference_mode);

  switch (GET_CODE (addr))
    {
    case POST_INC:
      ubi32_print_operand_address (file, XEXP (addr, 0));
      fprintf (file, "%d++", mode_size);
      break;
    ...
    case REG:
      fputc ('(', file);
      ubi32_print_reg (file, REGNO (addr));
      fputc (')', file);
      break;
	...
	if (CONST_INT_P (index))
	  {
	    ...
	    fputc ('(', file);
	    ubi32_print_reg (file, REGNO (base));
	  }
 		 ...
 		 || REG_P (index))
	  {
	    ...
	    ubi32_print_reg (file, REGNO (base));
	    fputc (',', file);
	    ubi32_print_reg (file, REGNO (index));
	  }
	else
	  abort ();
    ...
    case LO_SUM:
      fprintf (file, "%%lo(");
      ubi32_print_operand (file, XEXP (addr, 1), 'L');
      fprintf (file, ")(");
      ubi32_print_operand (file, XEXP (addr, 0), 0);
      fprintf (file, ")");
      break;
...
machine_mode
ubi32_select_cc_mode (enum rtx_code op, rtx x, rtx y)
{
...
bool
ubi32_match_cc_mode (rtx insn, machine_mode req_mode)
{
  rtx set;
  ...
  switch (req_mode)
    {
    case E_CCSZmode:
      if (set_mode != E_CCSZmode)
	return 0;
      break;

    case E_CCSZNmode:
      if (set_mode != E_CCSZmode
	  && set_mode != E_CCSZNmode)
	return 0;
      break;

    case E_CCSmode:
      if (set_mode != E_CCSmode
	  && set_mode != E_CCSZmode
	  && set_mode != E_CCSZNmode)
	return 0;
      break;
    ...
    case E_CCWZmode:
      if (set_mode != E_CCWZmode)
	return 0;
      break;

    case E_CCWZNmode:
      if (set_mode != E_CCWZmode
	  && set_mode != E_CCWZNmode)
	return 0;
      break;

    case E_CCWmode:
      if (set_mode != E_CCWmode
	  && set_mode != E_CCWZmode
	  && set_mode != E_CCWZNmode)
	return 0;
      break;
...
void
ubi32_canonicalize_comparison (enum rtx_code *code, rtx *op0, rtx *op1)
{
...
static bool
ubi32_fixed_condition_code_regs (unsigned int *p1, unsigned int *p2)
{
  *p1 = CC_REGNUM;
  *p2 = INVALID_REGNUM;
...
static machine_mode
ubi32_cc_modes_compatible (machine_mode m1, machine_mode m2)
{
  if (m1 == m2)
    return m1;
  ...
  switch (m1)
    {
    case E_CCWmode:
      if (m2 == E_CCWZNmode || m2 == E_CCWZmode)
	return m1;
    ...
    case E_CCWZmode:
      if (m2 == E_CCWmode || m2 == E_CCWZNmode)
	return m2;

      return VOIDmode;

    case E_CCSmode:
      if (m2 == E_CCSZNmode || m2 == E_CCSZmode)
	return m1;
    ...
    case E_CCSZmode:
      if (m2 == E_CCSmode || m2 == E_CCSZNmode)
	return m2;
...
static rtx
ubi32_legitimize_fdpic_address_symbol (rtx orig, rtx reg)
{
  rtx fdpic_reg;
...
static rtx
ubi32_legitimize_fdpic_address (rtx orig, rtx reg)
{
  rtx addr = orig;
  rtx new_rtx = orig;
  ...
  if (GET_CODE (addr) == CONST || GET_CODE (addr) == PLUS)
      ...
      if (GET_CODE (addr) == CONST)
	{
      ...
      base = ubi32_legitimize_fdpic_address_symbol (XEXP (addr, 0), reg);
...
rtx
ubi32_gen_compare (enum rtx_code code, rtx x, rtx y)
{
  machine_mode compare_mode = GET_MODE (x);
  ...
  switch (compare_mode)
	...
	break;
      ...
      if (! ubi32_data_register_operand (y, QImode))
	...
	break;
      ...
      if (! ubi32_data_register_operand (y, HImode))
	...
	break;
      ...
      if (! ubi32_data_register_operand (y, SImode))
	...
	break;
      ...
      if (! ubi32_data_register_operand (y, DImode))
  ...
  emit_insn (gen_rtx_SET (cc_reg, gen_rtx_COMPARE (cc_mode, x, y)));
...
void
ubi32_split_operand_pair (rtx operands[6])
{
...
{
  if (ubi32_data_register_operand (op0, SImode))
...
ubi32_gen_addsi3_ccwzn_null (rtx src1, rtx src2)
{
  ...
  compare = gen_rtx_COMPARE (E_CCWZNmode, neg, src2);
  cc_reg = gen_rtx_REG (E_CCWZNmode, CC_REGNUM);
  cc_set = gen_rtx_SET (cc_reg, compare);

  emit_insn (cc_set);
}

static void
ubi32_gen_cmpsi_sub4 (rtx src1, rtx src2)
{
  rtx compare;
  rtx cc_reg;
  rtx cc_set;

  compare = gen_rtx_COMPARE (E_CCWZNmode, src1, src2);
  cc_reg = gen_rtx_REG (E_CCWZNmode, CC_REGNUM);
  cc_set = gen_rtx_SET (cc_reg, compare);
...
void
ubi32_expand_movsi (rtx *operands)
{
  if (GET_CODE (operands[1]) == SYMBOL_REF
      ...
      if (TARGET_FDPIC)
	{
	  ...
	  if (GET_CODE (operands[1]) == SYMBOL_REF
	    ...
	    operands[1] = ubi32_legitimize_fdpic_address_symbol (operands[1], tmp);
	  else
	    operands[1] = ubi32_legitimize_fdpic_address (operands[1], tmp);
      ...
      if (can_create_pseudo_p() && ! REG_P (operands[0]))
	{
	  ...
	  emit_insn (gen_rtx_SET (tmp, operands[1]));
	  operands[1] = tmp;
	}
...
void
ubi32_expand_addsi3 (rtx *operands)
{
  rtx op, clob;

  if (can_create_pseudo_p ())
    {
      /* If we have a non-register for operand 1 then prefer a register.  */ 
      if (! register_operand (operands[1], GET_MODE (operands[1])))
	operands[1] = copy_to_mode_reg (SImode, operands[1]);

      if (CONST_INT_P (operands[2])
	  && ! satisfies_constraint_I (operands[2])
	  && ! satisfies_constraint_J1 (operands[2])
	  && ! satisfies_constraint_J2 (operands[2])
	  && ! satisfies_constraint_J4 (operands[2])
	  && ! satisfies_constraint_P4 (operands[2])
	  && ! (ubi32_v5 && satisfies_constraint_N4 (operands[2])))
	operands[2] = copy_to_mode_reg (SImode, operands[2]);
    }

  /* Emit the instruction.  */

  op = gen_rtx_SET (operands[0],
  ...
  if (! can_create_pseudo_p ())
    ...
    {
      clob = gen_rtx_CLOBBER (VOIDmode, gen_rtx_REG (E_CCmode, CC_REGNUM));
      emit_insn (gen_rtx_PARALLEL (VOIDmode, gen_rtvec (2, op, clob)));
    }
}

/* Expander for mulsi3 with a < v4 ISA.  */

static bool
ubi32_expand_mulsi3_v2 (rtx *operands)
{
  rtx a1, a1_1, a2;
  rtx b1, b1_1, b2;
  rtx mac_lo_rtx;
  ...
  if (!ubi32_data_register_operand (operands[1], GET_MODE (operands[1])))
    {
      rtx op1;
  ...
  if (!ubi32_data_register_operand (operands[2], GET_MODE (operands[2])))
    {
      rtx op2;
...
static bool
ubi32_expand_mulsi3_v4 (rtx *operands)
{
  rtx acc_rtx;
  ...
  /* Copy LSW of result to target.  */
  emit_move_insn (operands[0], 
		  gen_rtx_SUBREG (SImode, acc_rtx, ubi32_big_endian? 4: 0));
...
bool
ubi32_expand_mulsi3 (rtx *operands)
{
  /* Are we running with a >= v4 ISA?  If yes then the expansion is pretty
     straightforward.  */
  if (ubi32_v4)
    return ubi32_expand_mulsi3_v4 (operands);

  return ubi32_expand_mulsi3_v2 (operands);
}

/* Expander for umulhisi3.  */
bool
ubi32_expand_umulhisi3 (rtx *operands)
{
  rtx acc0_hi, acc1_hi;

  acc0_hi = gen_rtx_REG (HImode, ubi32_acc0_hi_regnum);
  acc1_hi = gen_rtx_REG (HImode, ubi32_acc1_hi_regnum);

  if (ubi32_v3)
    emit_insn (gen_umulhisi3_v3 (operands[0], operands[1], operands[2], acc0_hi, acc1_hi));
  else
    emit_insn (gen_umulhisi3_v2 (operands[0], operands[1], operands[2], acc0_hi));
  return 1;
}

/* Expander for mulhisi3.  */
bool
ubi32_expand_mulhisi3 (rtx *operands)
{
  rtx acc0_hi, acc1_hi;
  ...
  if (ubi32_v3)
    emit_insn (gen_mulhisi3_v3 (operands[0], operands[1], operands[2], acc0_hi, acc1_hi));
  else
    emit_insn (gen_mulhisi3_v2 (operands[0], operands[1], operands[2], acc0_hi));
  return 1;
}
...
ubi32_expand_seq (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  ...
  if (ubi32_arith_operand (cmp_op1, SImode)
      && ! ubi32_data_register_operand (cmp_op1, SImode)
      && ubi32_data_register_operand (cmp_op0, SImode))
    {
      rtx tmp = cmp_op0;
      cmp_op0 = cmp_op1;
      cmp_op1 = tmp;
  ...
  if (! ubi32_arith_operand (cmp_op0, SImode))
    {
  ...
  if (! ubi32_data_register_operand (cmp_op1, SImode))
    {
...
static bool
ubi32_expand_sne (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx scratch1;
  rtx sne;
...
static bool
ubi32_expand_sgt (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx slt;
  ...
  if (cmp_op1 == const0_rtx)
    {
      ...
      if (! ubi32_data_register_operand (cmp_op0, SImode))
	{
	  if (GET_MODE (cmp_op0) != SImode && GET_MODE (cmp_op0) != VOIDmode)
	    return false;
      ...
      ubi32_gen_lshrsi3 (operands[0], scratch2, GEN_INT (31));
      return true;
    }
  ...
  if (! ubi32_compare_operand (cmp_op0, SImode))
    {
      if (GET_MODE (cmp_op0) != SImode && GET_MODE (cmp_op0) != VOIDmode)
	return false;
...
static bool
ubi32_expand_slt (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx slt;

  if (cmp_op1 == const0_rtx)
    {
      ubi32_gen_lshrsi3 (operands[0], cmp_op0, GEN_INT (31));
      return true;
    }
  ...
  if (! ubi32_compare_operand (cmp_op1, SImode))
    {
      if (GET_MODE (cmp_op1) != SImode && GET_MODE (cmp_op1) != VOIDmode)
	return false;
...
static bool
ubi32_expand_sge (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sge;
  ...
  if (cmp_op1 == const0_rtx)
    {
      ...
      ubi32_gen_lshrsi3 (operands[0], scratch1, GEN_INT (31));
      return true;
    }
...
static bool
ubi32_expand_sle (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sge;
...
static bool
ubi32_expand_sgtu (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sltu;
  ...
  /* If we are comparing with zero then we can simplify this test to
     "sne".  */
  if (cmp_op1 == const0_rtx)
    return ubi32_expand_sne(cmp_op0, cmp_op1, operands);
...
static bool
ubi32_expand_sltu (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sltu;
...
static bool
ubi32_expand_sgeu (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sgeu;
...
static bool
ubi32_expand_sleu (rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  rtx sgeu;
  ...
  /* If we are comparing with zero then we can simplify this test to
     "seq".  */
  if (cmp_op1 == const0_rtx)
    return ubi32_expand_seq(cmp_op0, cmp_op1, operands);
...
bool
ubi32_expand_scode (enum rtx_code code, rtx cmp_op0, rtx cmp_op1, rtx *operands)
{
  ...
  switch (code)
    ...
    case GT:
      return ubi32_expand_sgt (cmp_op0, cmp_op1, operands);

    case LT:
      return ubi32_expand_slt (cmp_op0, cmp_op1, operands);

    case GE:
      return ubi32_expand_sge (cmp_op0, cmp_op1, operands);

    case LE:
      return ubi32_expand_sle (cmp_op0, cmp_op1, operands);

    case GTU:
      return ubi32_expand_sgtu (cmp_op0, cmp_op1, operands);

    case LTU:
      return ubi32_expand_sltu (cmp_op0, cmp_op1, operands);

    case GEU:
      return ubi32_expand_sgeu (cmp_op0, cmp_op1, operands);

    case LEU:
      return ubi32_expand_sleu (cmp_op0, cmp_op1, operands);
		       ...
		       rtx dest, rtx src, rtx increment)
{
  ...
  if (! ubi32_data_register_operand (src, SImode))
    {
  ...
  if (increment == const1_rtx)
    {
      ...
      ubi32_gen_cmpsi_sub4 (const0_rtx, scratch);
  ...
  else
    {
      ...
      ubi32_gen_addsi3_ccwzn_null (scratch, constm1_rtx);
...
static bool
ubi32_expand_addne (rtx cmp_op0, rtx cmp_op1,
		       rtx dest, rtx src, rtx increment)
{
  rtx sub_1;
  rtx scratch;
...
bool
ubi32_expand_addcc (enum rtx_code code, rtx cmp_op0, rtx cmp_op1,
		       rtx dest, rtx src, rtx increment)
{
  switch (code)
    {
    case EQ:
      return ubi32_expand_addeq (cmp_op0, cmp_op1, dest, src, increment);

    case NE:
      return ubi32_expand_addne (cmp_op0, cmp_op1, dest, src, increment);
...
ubi32_split_slt (rtx *operands)
{
  ...
  if (MEM_P (operands[2])
      || (REG_P (operands[2])
	  && !ubi32_data_register_operand (operands[2],
					      GET_MODE (operands[2]))))
    {
...
ubi32_split_sltu (rtx *operands)
{
  ...
  if (ubi32_data_register_operand (operands[6], GET_MODE (operands[6])))
...
void
ubi32_emit_move_const_int (rtx dest, rtx imm)
{
  rtx xoperands[2];
  ...
  if (MEM_P (dest))
      ...
      if (satisfies_constraint_I (imm))
	{
      ...
      /* See if we can turn our single write into two writes that are half
         the size.  */
      if (! MEM_VOLATILE_P (dest))
	{
	  if (ubi32_big_endian)
	      ...
	      if (ubi32_legitimate_address_p (HImode, high_hword_addr, 1))
		{
		  ...
		  MEM_COPY_ATTRIBUTES (high_hword_mem, dest);
		  output_asm_insn ("movei\t%0, #%%lo(%E1)", xoperands);
		  xoperands[0] = high_hword_mem;
		  output_asm_insn ("movei\t%0, #%%hi(%E1)", xoperands);
		  return;
		}
	    }
      ...
      if (! reg_mentioned_p (stack_pointer_rtx, dest))
	{
	  if (ubi32_big_endian)
	    ...
	    {
	      output_asm_insn ("movei\t-4(sp)++, #%%lo(%E1)", xoperands);
	      output_asm_insn ("movei\t2(sp), #%%hi(%E1)", xoperands);
	    }
...
static void
ubi32_layout_prologue_frame (int adj)
{
  rtx_insn *insn;
  ...
  already_saved[LINK_REGNUM] = 1;
  already_saved[HARD_FRAME_POINTER_REGNUM] = 1;
  ...
  if (crtl->is_leaf && (! df_regs_ever_live_p (LINK_REGNUM)))
    save_link_reg = false;
  else
    {
      save_link_reg = true;
      save_regs_order[nregs] = LINK_REGNUM;
      nregs++;
    }

  if (frame_pointer_needed || df_regs_ever_live_p (HARD_FRAME_POINTER_REGNUM))
    {
      save_regs_order[nregs] = HARD_FRAME_POINTER_REGNUM;
      nregs++;
    }
...
static void
ubi32_expand_prologue_1 (int adj)
{
  rtx addr;
  rtx mem;
  rtx reg;
  ...
  addr = gen_rtx_PRE_MODIFY (Pmode,
			     stack_pointer_rtx,
			     plus_constant (Pmode, stack_pointer_rtx, -(adj + save_regs_size)));
...
static void
ubi32_expand_prologue_3 (int adj)
{
  rtx addr;
  rtx mem;
  rtx reg;
...
ubi32_expand_prologue (void)
{
  ...
  if (save_regs)
	...
	{
	case 1:
	  ubi32_expand_prologue_1 (adj);
	  break;

	case 2:
	  ubi32_expand_prologue_2 (adj);
	  break;

	case 3:
	  ubi32_expand_prologue_3 (adj);
	  break;
  ...
  if (frame_pointer_needed)
    {
      ...
      x = gen_addsi3 (hard_frame_pointer_rtx,
		      stack_pointer_rtx,
		      GEN_INT (fp_adj));
      x = emit_insn (x);
      RTX_FRAME_RELATED_P (x) = 1;
...
static bool
ubi32_expand_epilogue_1 (int adj, bool sibcall)
{
  int i;
  unsigned int regno;
  rtx reg;
  ...
  if (regno == LINK_REGNUM && ! cfun->stdarg && ! sibcall)
    {
  ...
  if (sibcall
      && save_link_reg
      && regno == LINK_REGNUM)
    emit_use (gen_rtx_REG (SImode, LINK_REGNUM));

  return false;
}

static bool
ubi32_expand_epilogue_2 (int adj, bool sibcall)
{
  int i;
  rtx x;
...
static bool
ubi32_expand_epilogue_3 (int adj, bool sibcall)
{
  int i;
  unsigned int regno;
  rtx reg;
  ...
  addr = gen_rtx_POST_MODIFY (Pmode,
			      stack_pointer_rtx,
			      plus_constant (Pmode, stack_pointer_rtx, adj + 4));
  mem = gen_frame_mem (SImode, addr);
...
void
ubi32_expand_epilogue (bool sibcall)
{
  rtx x;
  int outgoing_args_size = crtl->outgoing_args_size;
  int adj;
  ...
  if (save_regs)
	...
	{
	case 1:
	  if (ubi32_expand_epilogue_1 (adj, sibcall))
	    return;
	  break;

	case 2:
	  if (ubi32_expand_epilogue_2 (adj, sibcall))
	    return;
	  break;

	case 3:
	  if (ubi32_expand_epilogue_3 (adj, sibcall))
	    return;
	  break;
	}
  ...
  if (! sibcall)
    emit_jump_insn (gen_return_internal (gen_rtx_REG (SImode, LINK_REGNUM)));
}

void
ubi32_expand_eh_return (rtx *operands)
{
  if (REG_P (operands[0])
...
static bool
ubi32_function_ok_for_sibcall (tree decl ATTRIBUTE_UNUSED, tree exp ATTRIBUTE_UNUSED)
{
...
int
ubi32_initial_elimination_offset (int from, int to)
{
  int i;
  int nregs = 0;
  ...
  if (from == FRAME_POINTER_REGNUM && to == STACK_POINTER_REGNUM)
	...
	/* return (4 * nregs) + crtl->outgoing_args_size + get_frame_size(); */
	return (4 * nregs) + crtl->outgoing_args_size + get_frame_size();
      else
	return (4 * nregs) + crtl->outgoing_args_size;
    }
  else if (from == FRAME_POINTER_REGNUM && to == HARD_FRAME_POINTER_REGNUM)
    {
      if (FRAME_GROWS_DOWNWARD)
	/* return (4 * nregs) + crtl->outgoing_args_size + get_frame_size(); */
	return crtl->outgoing_args_size + get_frame_size();
      else
	return 0;
    }
...
static tree
ubi32_handle_fndecl_attribute (tree *node, tree name,
...
enum reg_class
ubi32_preferred_reload_class (rtx x, enum reg_class rclass)
{
...
void
ubi32_init_cumulative_args (CUMULATIVE_ARGS *cum, 
...
static void
ubi32_function_arg_advance (cumulative_args_t cum_v, machine_mode mode,
...
static rtx
ubi32_function_arg (cumulative_args_t cum_v, machine_mode mode,
...
/* Implement hook TARGET_ARG_PARTIAL_BYTES.
   ...
   on the stack.  */
static int
ubi32_arg_partial_bytes (cumulative_args_t pcum_v, machine_mode mode,
			    tree type, bool named ATTRIBUTE_UNUSED)
{
  ...
  pcum->nbytes = (pcum->nbytes + 3) & ~3;

  if (targetm.calls.pass_by_reference (pcum_v, mode, type, named))
      return 0;

  /* number of bytes left in registers */
  diff = nregs * UNITS_PER_WORD - pcum->nbytes;
...
static bool
ubi32_pass_by_reference (cumulative_args_t pcum_v ATTRIBUTE_UNUSED,
...
static bool
ubi32_callee_copies (cumulative_args_t pcum_v ATTRIBUTE_UNUSED,
		     machine_mode mode, const_tree type,
		     bool named ATTRIBUTE_UNUSED)
{
  int size;
...
static bool
ubi32_return_in_memory (const_tree type, const_tree fntype ATTRIBUTE_UNUSED)
{
  int size;
...
/* Return true if a given register number REGNO is acceptable for machine
   mode MODE.  */
bool
ubi32_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
...
int
ubi32_regno_ok_for_base_p (int regno, int strict)
{
  if ((regno >= FIRST_ADDRESS_REGNUM && regno <= LAST_ADDRESS_REGNUM)
      || (!strict
      ...
      || (strict && (reg_renumber
		     ...
		     && reg_renumber[regno] <= LAST_ADDRESS_REGNUM)))
    return 1;
...
int
ubi32_regno_ok_for_index_p (int regno, int strict)
{
...
static bool
ubi32_is_index_reg (rtx x, int strict)
{
  if ((REG_P (x) && ubi32_regno_ok_for_index_p (REGNO (x), strict))
      || (GET_CODE (x) == SUBREG
	  && REG_P (SUBREG_REG (x))
	  && ubi32_regno_ok_for_index_p (REGNO (SUBREG_REG (x)), strict)))
    return true;
...
static bool
ubi32_is_index_expr (machine_mode mode, rtx x, int strict)
{
  ...
  if (CONST_INT_P (x))
	...
	{
	case E_QImode:
	  return satisfies_constraint_J1 (x);

	case E_HImode:
	  return satisfies_constraint_J2 (x);

	case E_SImode:
	case E_SFmode:
	  return satisfies_constraint_J4 (x);

	case E_DImode:
	case E_DFmode:
	  return satisfies_constraint_J4 (x)
		 && satisfies_constraint_J4 (GEN_INT (INTVAL (x) + 4));
...
static bool
ubi32_is_valid_offset (machine_mode mode, HOST_WIDE_INT offs)
{
  if (offs < 0)
    return false;
...
static int
ubi32_get_valid_offset_mask (machine_mode mode)
{
  switch (mode)
    {
    case E_QImode:
...
static bool
ubi32_is_base_reg (rtx x, int strict)
{
  if ((REG_P (x) && ubi32_regno_ok_for_base_p (REGNO (x), strict))
      || (GET_CODE (x) == SUBREG
	  && REG_P (SUBREG_REG (x))
	  && ubi32_regno_ok_for_base_p (REGNO (SUBREG_REG (x)), strict)))
    return true;
...
static bool
ubi32_cannot_force_const_mem (machine_mode mode ATTRIBUTE_UNUSED,
...
static bool
ubi32_legitimate_constant_p (machine_mode mode ATTRIBUTE_UNUSED, rtx x)
{
...
bool
ubi32_legitimate_address_p (machine_mode mode, rtx x, int strict)
{
  if (CONSTANT_ADDRESS_P (x))
    return false;

  if (ubi32_is_base_reg (x, strict))
    return true;
...
rtx
ubi32_legitimize_address (rtx x, rtx oldx ATTRIBUTE_UNUSED,
			     machine_mode mode)
{
  if (mode == E_BLKmode)
    return x;
  ...
  if (GET_CODE (x) == PLUS
      ...
      && ! REGNO_PTR_FRAME_P (REGNO (XEXP (x, 0)))
      && CONST_INT_P (XEXP (x, 1))
      && ! ubi32_is_valid_offset (mode, INTVAL (XEXP (x, 1))))
    {
      rtx base;
      rtx plus;
      rtx new_rtx;
      HOST_WIDE_INT val = INTVAL (XEXP (x, 1));
      HOST_WIDE_INT low = val & ubi32_get_valid_offset_mask (mode);
...
rtx
ubi32_legitimize_reload_address (rtx ad, machine_mode mode,
				    int opnum, int type)
{
  ...
  if (GET_CODE (ad) == PLUS
      ...
      && CONST_INT_P (XEXP (ad, 1))
      && ! ubi32_is_valid_offset (mode, INTVAL (XEXP (ad, 1))))
    {
      rtx temp;
      rtx new_rtx;
...
static int
ubi32_address_cost (rtx x, machine_mode mode ATTRIBUTE_UNUSED,
		  addr_space_t as ATTRIBUTE_UNUSED, bool speed ATTRIBUTE_UNUSED)
{
  ...
  if (c == REG)
    {
	  ...
	  && ((r < FIRST_ADDRESS_REGNUM)
	      || (r > LAST_ADDRESS_REGNUM)))
	cost += 10;
  ...
  if (c == PLUS
	...
	{
	  if (!ubi32_is_valid_offset (GET_MODE (x), INTVAL (XEXP (x, 1))))
	    cost += 5;
	}
      else
	cost += 10;
...
static bool
ubi32_rtx_costs (rtx x, machine_mode mode, int outer_code,
  ...
  switch (code)
	  ...
	  && exact_log2 (0xffffffff & INTVAL (x)) != -1)
	{
	  *total = 0;
      ...
      /* Larger constants that can be loaded via movei aren't too bad.  */
      if (satisfies_constraint_N1 (x))
	{
      ...
      /* Larger constants that can be loaded via bset aren't too bad.  */
      if (exact_log2 (0xffffffff & INTVAL (x)) != -1)
	{
	  if (mode == DImode)
	    *total = COSTS_N_INSNS (2);
	  else
	 ...
	 more expensive than we might otherwise want to.  */
      if (mode == SImode
	  && (((ubi32_v5 ? 0xffffff80 : 0x7fffff80) & INTVAL (x))
	      == (0xffffffff & INTVAL (x))))
	{
...
int
ubi32_mode_dependent_address_p (rtx addr)
{
...
static void
ubi32_function_prologue (FILE *file)
{
  fprintf (file, "/* frame/pretend: %ld/%d save_regs: %d out_args: %d  %s */\n",
	   (long) get_frame_size (), crtl->args.pretend_args_size,
	   save_regs_size, (int) crtl->outgoing_args_size,
	   crtl->is_leaf ? "leaf" : "nonleaf");
}

static void
ubi32_function_epilogue (FILE *file ATTRIBUTE_UNUSED)
{
}
...
ubi32_safe_attr_type (rtx_insn *insn)
{
...
static int
ubi32_safe_attr_nop (rtx_insn* insn)
{
  enum attr_nop an;
  if (recog_memoized (insn) < 0)
    return 0;
...
ubi32_hazard_scan (basic_block bb, int range, int addr_pad, int mac_pad, int fpu_pad)
{
  ...
  FOR_BB_INSNS (bb, def_insn)
      ...
      for (j = 0; j < range; j++)
	{
	  ...
	  if (ubi32_safe_attr_type (use_insn) == TYPE_NOP)
	    {
	      j += ubi32_safe_attr_nop (use_insn) - 1;
	      continue;
	    }
	...
	continue;
      ...
      if (mac_pad > 0)
	{
	  if (ubi32_multiply_dep_p (def_insn, use_insn))
	    {
      ...
      if (addr_pad > 0)
	{
	  if (ubi32_address_dep_p (def_insn, use_insn))
	    {
	      insert_before = use_insn;
      ...
      if (fpu_pad > 0)
	{
	  if (ubi32_fpu_dep_p (def_insn, use_insn))
	    {
	      insert_before = use_insn;
	      nop_padding = fpu_pad;
	    }
...
ubi32_get_mem_refs (rtx x)
{
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e'
	  && ubi32_get_mem_refs (XEXP (x, i)))
	refs++;
      else if (fmt[i] == 'E')
	for (j = 0; j < XVECLEN (x, i); j++)
	  if (ubi32_get_mem_refs (XVECEXP (x, i, j)))
	    refs++;
    }
...
ubi32_get_overcommits_1 (rtx r)
{
  ...
  /* We may need to account for more than a single word write here, but
     for now we keep this simple.  */
  reads = ubi32_get_mem_refs (SET_SRC (r)) ? 1 : 0;
  writes = (MEM_P (SET_DEST (r))) ? 0 : -1;
  return reads + writes;
}
...
static int
ubi32_get_overcommits (rtx insn)
{
  rtx i_rtx;
  ...
  if (GET_CODE (i_rtx) == SET)
    return ubi32_get_overcommits_1 (i_rtx);
  ...
  if (GET_CODE (i_rtx) == PARALLEL)
	...
	if (GET_CODE (XVECEXP (i_rtx, 0, i)) == SET)
	  return ubi32_get_overcommits_1 (XVECEXP (i_rtx, 0, i));
    }
...
ubi32_machine_dependent_reorg (void)
{
  ...
  if (ubi32_v5)
    {
      addr_ext = 5 / ubi32_sched_rate;
      mac_ext = 2 / ubi32_sched_rate;
      if (TARGET_HARD_FLOAT)
	fpu_ext = 5 / ubi32_sched_rate;
      else
	fpu_ext = 0;
    ...
    {
      addr_ext = 4 / ubi32_sched_rate;
      mac_ext = 3 / ubi32_sched_rate;
      fpu_ext = 0;
      largest_ext = addr_ext;
    }
  ...
  FOR_EACH_BB_FN (bb, cfun)
	...
	ubi32_hazard_scan(bb, i + 1, addr_ext - i , mac_ext - i,
			     fpu_ext - i);
    }

  /* Ubi32 v3, v4, v5 and v6 are all implemented with a write buffer that
     can overfill at a scheduling rate of 1.  Look at whether we should
     try to work-around that with nop padding.  */
  if ((ubi32_v3 || ubi32_v4 || ubi32_v5 || ubi32_v6 || ubi32_v61)
      && ubi32_sched_rate == 1)
    FOR_EACH_BB_FN (bb, cfun)
...
rtx
ubi32_return_addr_rtx (int count, rtx frame ATTRIBUTE_UNUSED)
{
  if (count != 0)
    return NULL_RTX;
...
static rtx
ubi32_expand_builtin_1t_1s (enum insn_code icode, tree exp, rtx target)
{
  tree arg0;
  rtx op0;
  rtx pat;
...
/* Given a builtin function taking two source operands and one target operands,
   emit the RTL for the underlying instruction. */
static rtx
ubi32_expand_builtin_1t_2s (enum insn_code icode, tree exp, rtx target)
{
  tree arg0;
  tree arg1;
  rtx op0;
...
ubi32_expand_builtin_spinlock_lock (tree exp)
{
  ...
  if (prev_insn)
    {
      ...
      if (prev_set
	  && rtx_equal_p (SET_DEST (prev_set), addr))
	addr = SET_SRC (prev_set);
    }
  ...
  /* Grab the incoming argument and emit its RTL. */
  op1 = expand_normal (CALL_EXPR_ARG (exp, 1));
  if (! CONST_INT_P (op1))
    return NULL_RTX;
...
/* Expand the RTL for spinlock_unlock.  */
static rtx
ubi32_expand_builtin_spinlock_unlock (tree exp)
{
  ...
  /* Emit and return the new instruction. */
  emit_insn (gen_andsi3_bclr (mem, mem, GEN_INT (bmask)));

  return const0_rtx;
}

/* Expand the RTL for spinlock_is_locked.  */
static rtx
ubi32_expand_builtin_spinlock_is_locked (tree exp, rtx target)
{
  ...
  if (! target
      ...
      || ! ubi32_data_register_operand (target, SImode))
  ...
  /* Emit and return the new instruction(s). */
  bit = INTVAL (op1);
  if (bit == 0)
    emit_insn (gen_andsi3 (target, mem, const1_rtx));
  else if (bit == 31)
    emit_insn (gen_lshrsi3 (target, mem, op1));
  else
    {
      rtx tmp = gen_reg_rtx (SImode);
      emit_insn (gen_lshrsi3 (tmp, mem, op1));
      emit_insn (gen_andsi3 (target, tmp, const1_rtx));
    }

  return target;
}

/* Expand the RTL for spinlock_try_lock.  */
static rtx
...
static rtx
ubi32_expand_builtin (tree exp, rtx target, rtx subtarget ATTRIBUTE_UNUSED,
			 ...
			 int ignore ATTRIBUTE_UNUSED)
{
  ...
  switch (fcode)
    {
    case UBI32_BUILTIN_UBI32_SWAPB_2:
    ...
    case UBI32_BUILTIN_SPINLOCK_UNLOCK:
      return ubi32_expand_builtin_spinlock_unlock (exp);

    case UBI32_BUILTIN_SPINLOCK_TRY_LOCK:
      return ubi32_expand_builtin_spinlock_try_lock (exp, target);

    case UBI32_BUILTIN_SPINLOCK_IS_LOCKED:
      return ubi32_expand_builtin_spinlock_is_locked (exp, target);
  ...
  if (TREE_CODE (arg0) == INTEGER_CST)
    {
      ...
      return wide_int_to_tree (TREE_TYPE (TREE_TYPE (fndecl)), res);
    }
		    ...
		    bool ignore ATTRIBUTE_UNUSED)
{
  switch (DECL_FUNCTION_CODE (fndecl))
    {
    case UBI32_BUILTIN_UBI32_SWAPB_2:
      return ubi32_fold_builtin_ubi32_swapb_2 (fndecl, arglist);
...
/* Implementation of TARGET_ASM_INTEGER.  When using FD-PIC, we need to
   ...
   some cases.  */
static bool
ubi32_assemble_integer (rtx value, unsigned int size, int aligned_p)
{
...
int
ubi32_shiftable_const_int (int i)
{
  int shift = 0;
...
int
ubi32_memory_move_cost (machine_mode mode,
			   enum reg_class regclass ATTRIBUTE_UNUSED,
			   int in)
{
  /* If we have a v1 or v2 ISA device then the data memory is an on-chip
     SRAM and memory moves are really cheap!  If we're optimizing for size
     on the ohter ISA devices it's cheaper in terms of instruction size
     to pretend these are cheap too.  */
  if (optimize_size || (! ubi32_v3))
    {
      if (mode == DImode || mode == DFmode)
...
/* Return the cost of moving data from a register in class CLASS1 to
   one in class CLASS2.  */
int
ubi32_register_move_cost (machine_mode mode ATTRIBUTE_UNUSED,
...
static bool
ubi32_reg_used_in_mem_of_rtx_p (unsigned int regno, rtx x)
{
  enum rtx_code code = GET_CODE (x);
  int i, j;
  const char *fmt;
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e'
	  && ubi32_reg_used_in_mem_of_rtx_p (regno, XEXP (x, i)))
	return true;

      if (fmt[i] == 'E')
	for (j = 0; j < XVECLEN (x, i); j++)
	  if (ubi32_reg_used_in_mem_of_rtx_p (regno, XVECEXP (x, i, j)))
	    return true;
    }
...
/* Returns true if expression DEP_RTX sets a register
   used by instruction INSN to address memory.  */

static bool
ubi32_addr_generation_dependency_p (rtx dep_rtx, rtx_insn *insn)
{
  rtx i_rtx;
  rtx target;
  unsigned int regno;
  ...
  /* If we have an LEA-like instruction then we want to try
     ...
     early.  */
  if (ubi32_safe_attr_type (insn) == TYPE_LEA)
    {
  ...
  if (ubi32_safe_attr_type (insn) == TYPE_CALL)
    {
  ...
  if (GET_CODE (i_rtx) == SET
      && ubi32_address_register_operand (SET_DEST (i_rtx), VOIDmode)
      && REG_P (SET_SRC (i_rtx))
      && REGNO (SET_SRC (i_rtx)) == regno)
    return true;

  return ubi32_reg_used_in_mem_of_rtx_p (regno, i_rtx);
}

/* Return 1 if DEP_INSN sets register used in INSN in the agen unit.  */

int
ubi32_address_dep_p (rtx_insn *dep_insn, rtx_insn *insn)
{
  rtx dep_rtx;
  ...
  if (ubi32_safe_attr_type (dep_insn) == TYPE_LEA)
    return 0;
  ...
  if (GET_CODE (dep_rtx) == SET
      && ubi32_address_register_operand (SET_DEST (dep_rtx), VOIDmode)
  ...
  /* Is the instruction on which we might depend a simple set?  If yes
     then see if we have a dependency.  */
  if (GET_CODE (dep_rtx) == SET
      && ubi32_addr_generation_dependency_p (dep_rtx, insn))
    return 1;
  ...
  if (GET_CODE (dep_rtx) == PARALLEL)
	...
	if (ubi32_addr_generation_dependency_p (XVECEXP (dep_rtx, 0, i), insn))
	  return 1;
    }
...
static bool
ubi32_multiply_dependency_p (rtx dep_rtx, rtx insn)
{
  rtx i_rtx;
  rtx target;
  unsigned int regno;
  ...
  regno = REGNO (target);
  if (regno != ubi32_acc0_hi_regnum
      && regno != ubi32_acc0_lo_regnum
      && regno != ubi32_acc1_hi_regnum
      && regno != ubi32_acc1_lo_regnum)
    return false;
  ...
  /* Accumulator reg numbers change depending on endianity. 
     Pick lowest number no matter which endianity.  */

  #define min(a,b) ((a > b) ? b : a)
  if (regno == ubi32_acc0_lo_regnum || regno == ubi32_acc0_hi_regnum)
    regno = min (ubi32_acc0_lo_regnum, ubi32_acc0_hi_regnum);
  else if (regno == ubi32_acc1_lo_regnum || regno == ubi32_acc1_hi_regnum)
    regno = min (ubi32_acc1_lo_regnum, ubi32_acc1_hi_regnum);
...
static int
ubi32_multiply_dep_p (rtx dep_insn, rtx insn)
{
  rtx dep_rtx;
  ...
  /* Is the instruction on which we might depend a simple set?  If yes
     then see if we have a dependency.  */
  if (GET_CODE (dep_rtx) == SET
      && ubi32_multiply_dependency_p (dep_rtx, insn))
    return 1;
  ...
  if (GET_CODE (dep_rtx) == PARALLEL)
    {
      ...
      for (i = 0; i < XVECLEN (dep_rtx, 0); i++)
	if (ubi32_multiply_dependency_p (XVECEXP (dep_rtx, 0, i), insn))
	  return 1;
    }
...
static bool
ubi32_fpu_dependency_p (rtx dep_rtx, rtx insn)
{
  rtx i_rtx;
  rtx target;
  unsigned int regno;
...
ubi32_fpu_dep_p (rtx dep_insn, rtx insn)
{
  ...
  if (ubi32_safe_attr_type (dep_insn) != TYPE_FPU)
    return 0;
  ...
  /* Is the instruction on which we might depend a simple set?  If yes
     then see if we have a dependency.  */
  if (GET_CODE (dep_rtx) == SET
      && ubi32_fpu_dependency_p (dep_rtx, insn))
    return 1;
...
bool
ubi32_hard_regno_rename_ok (unsigned int from ATTRIBUTE_UNUSED, unsigned int to)
{
  ...
  if (to == LINK_REGNUM && !save_link_reg)
    return false;
...
#define TARGET_ASM_RELOC_RW_MASK ubi32_reloc_rw_mask

#undef TARGET_ASM_FUNCTION_PROLOGUE
#define TARGET_ASM_FUNCTION_PROLOGUE ubi32_function_prologue

#undef TARGET_ASM_FUNCTION_EPILOGUE
#define TARGET_ASM_FUNCTION_EPILOGUE ubi32_function_epilogue

#undef TARGET_ASM_FUNCTION_RODATA_SECTION
#define TARGET_ASM_FUNCTION_RODATA_SECTION ubi32_function_rodata_section

#undef TARGET_ATTRIBUTE_TABLE
#define TARGET_ATTRIBUTE_TABLE ubi32_attribute_table

#undef TARGET_ADDRESS_COST
#define TARGET_ADDRESS_COST ubi32_address_cost

#undef TARGET_RTX_COSTS
#define TARGET_RTX_COSTS ubi32_rtx_costs

#undef TARGET_FIXED_CONDITION_CODE_REGS
#define TARGET_FIXED_CONDITION_CODE_REGS ubi32_fixed_condition_code_regs
...
#define TARGET_CALLEE_COPIES ubi32_callee_copies
...
#define TARGET_EXPAND_BUILTIN ubi32_expand_builtin
...
#ifdef FIXME  /* How is this generated? */
#include "gt-ubi32.h"
#endif
